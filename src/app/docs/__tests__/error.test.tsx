...
describe("DocsError", () => {
  it("logs the error and offers a retry that calls unstable_retry", async () => {
    ...
    render(<DocsError error={error} unstable_retry={unstable_retry} />);
  ...
  it("omits the digest line when the error has none", () => {
    render(<DocsError error={new globalThis.Error("boom")} unstable_retry={vi.fn()} />);
